...
client.graphql.on('cache', reportCacheErrors) // our first query will requests all movies 
...
export const MOVIE_QUERY = `
 ...
 `
 // This is generic query function // We will use this with one of the above queries and 
 // variables if needed export async function get(query, variables = null) {     const response =  await client.get({ query , variables })     console.log("response", response);     return response }